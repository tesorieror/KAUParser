...
function parseDict(f, overwrite) {
	...
	function addLinesToDictionary(lines) {
	...
	function addToDictionaryBase(lines) {
		...
		return addLinesToDictionary(lines);
	}
	...
	// if (line[2]) {
	// console.log('ID found for line: ', JSON.stringify(line));
	// return line[2];
	// }
	...
	function toCSV(lines) {
		return _.reduce(lines, function(result, line) {
			...
			line[2] = createAchronym(line);
			 ...
			 if (line[2]) {
			 console.log('Key found ' + line[2]);
			// // if (dictionaryByID[line[2]]) {
			// // console.error('Line ' + line);
			// // throw Error('Duplicated key ' + line[2] + ' for ' + line[1]+' and
			// '+dictionaryByID[line[2]]);
			  }
			// } else {
			// line[2] = createAchronym(line);
			// }
		...
		.then(function(data) {
			...
			for (var r = row; r < indicators.length; r++) {
				...
				for (var c = offset; c < col + offset; c++) {
					...
					// console.log("Indicator 2973", JSON.stringify(result[result.length -
					// 1]));
					// }
					// if (result.length == 2978) {
					// console.log("Indicator 2978", JSON.stringify(result[result.length -
					// 1]));
					// console.log("Indicator 2978 (raw)", JSON.stringify(indicators[r]));
					// console.log("Indicator 2978 [7] h ",
					// JSON.stringify(indicators[1][7]));
					// console.log("Indicator 2978 [7] ",
					// JSON.stringify(indicators[r][7]));
					// var catTemp = indicators[1][hcol];
					// console.log("CatTemp: ", catTemp);
					// console.log("Indicator 2978 [8] h ",
					// JSON.stringify(indicators[1][8]));
					// console.log("Indicator 2978 [8] ",
					// JSON.stringify(indicators[r][8]));
